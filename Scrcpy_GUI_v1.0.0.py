...
root.iconbitmap(os.path.join(BASE_DIR, "Scrcpy_GUI_v1.0.0.ico"))
...
def minimize_app():
    root._iconified = True
    root.overrideredirect(False)
    root.iconify()

def on_deiconify(event):
    if getattr(root, '_iconified', False) and root.state() == "normal":
        root._iconified = False
        root.overrideredirect(True)
...
minimize_btn = tk.Button(titlebar, text="−", bg="#212529", fg="#adb5bd", font=("Segoe UI", 12, "bold"), bd=0, command=minimize_app, width=3, height=1)
minimize_btn.pack(side="right", padx=0, pady=0)
minimize_btn.configure(activebackground="#495057", activeforeground="white")
...
for widget in [titlebar, title_label, close_btn, minimize_btn]:
...
def check_binaries():
    ...
    if not adb_exists and not scrcpy_exists:
        status_label.configure(text="ADB & Scrcpy not installed", bootstyle=DANGER)
        binaries_ok = False
    elif not adb_exists:
        status_label.configure(text="ADB not installed", bootstyle=DANGER)
        binaries_ok = False
    elif not scrcpy_exists:
        status_label.configure(text="Scrcpy not installed", bootstyle=DANGER)
        binaries_ok = False
    if not binaries_ok:
...
def select_apk():
    ...
    if path:
        ...
        if target or last_ip:
            ...
            status_label.configure(text="No device connected", bootstyle=DANGER)
...
for var in [video_var, audio_var, control_var, hid_var, gamepad_var, borderless_var, fullscreen_var, screenoff_var, auto_resize_var, recorder_var, save_path_var, app_launch_var, res_w_var, res_h_var, dpi_var]:
